...
* Understanding how community weighted means (CWM) are calculated
* Ability to calculate CWMs with the example of the functional trait specific leaf area (SLA)
* Knowledge that one can do the same with Ellenberg values
...
* Calculate CWMs of SLA
  * Check that species names of the traits table are rownames
  * Check that only the SLA column remains in the traits table
  * Check that no NAs are in your SLA column
