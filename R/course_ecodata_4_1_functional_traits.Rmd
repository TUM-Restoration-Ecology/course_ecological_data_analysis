...
* Make script run
* check new traits table
* Calculate functional traits yourself brick-trees dataset
* Select also LMF, SRL, LeafN

This script uses parts of the vignette of Laughlin et al. ([2018](https://cran.r-project.org/web/packages/Select/vignettes/selectSpecies.html))

# 1 Preparation

## 1.1 Load libraries

You can always install missing libraries by `install.packages("here")` for the `here` package. The tidyverse package is of Wickham et al. ([2019](https://doi.org/10.21105/joss.01686))
...
* Search the IDs of *mean* specific leaf area (SLA), seed mass and plant height.
...
* Get the aggregated trait values of the mean SLA, plant height and seed mass.

Here is an example how to download several traits:

```{r get-traits, results = FALSE, message = FALSE}
trait_ids <- c(
  "1.2.2", "1.5", "1.6.3", "3.2.3", "3.3.1", "3.6.1", "3.7.1", "3.21.1",
  "4.1.3", "4.5.1"
  )
trait_meta %>%
...
```{r save-files}
write_csv(species, here("data", "processed", "data_processed_species_ammer.csv"))
write_csv(traits, here("data", "processed", "data_processed_traits_ammer.csv"))
```
