---
title: "Analysis of Ecological Data <br> 3.4 Community Weighted Mean"
...
### 2.1.3. Calculation with a package
...
```{r}
rownames(species_spain) == rownames(spxt)
```
...
```{r}
resCWM <- dbFD(spxt, log(t(species_spain) + 1), CWM.type = "all")
...
We can already reach some conclusions while looking at these figures. The CWM SLA is increasing with moisture index, likely because of a decrease in shrub species with moisture. If fact, the different growth forms tend to have different SLA. You can test this by using this line:
