...
duplicates <- data %>% filter(duplicated(accepted_name))
...
You can work with classical Ellenberg indicator values (EIV) like with functional plant traits. Tichý et al. ([2023](https://doi.org/10.1111/jvs.13168)) provide European Ellenberg values on [Zenodo](https://doi.org/10.5281/zenodo.7427088). The `vegdata` package stores the German Ellenberg values, but it does not always work.

```{r save-files, results = FALSE}
library(vegdata)
tv_home <- options(
  tv_home = here("data", "raw", "GermanSL 1.5")
  )
tv_home
ellenberg <- vegdata::tv.traits()
```
