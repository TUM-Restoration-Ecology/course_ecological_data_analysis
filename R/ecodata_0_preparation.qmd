...
We will meet on Monday (**03.03.2025**) at **9 am** in the forestry building of the Weihenstephan-Triesdorf University of Applied Sciences (**Room F9.354**, Hans-Carl-von-Carlowitz-Platz 3, Freising).
...
It is possible to evaluate your own datasets directly during the course. If you do not have your own data sets, we will provide you with sample data sets. If you need a dataset, please inform us until the **21.02.2025**. If you have a dataset format the dataset according to the instruction file **before the course starts** (create a species and environment matrix). Please ask us, if the dataset you want to use is suitable.
...
The exam will take place on the last day of the course (Friday, **07.03.2025**) and consists of a short presentation in which you will present your own evaluations. 
...
* MI 9:00-17:00 (1h break): Functional trait diversity: Community-weighted mean
* DO 9:00-17:00 (1h break): Species/trait diversity ~ environment
* FR 9:00-13:00 (0h break): Exam
...
With the first mail you receive a project template called 'course_ecodata_my_project' with the R project called 'my_project.Rproj'. Scripts are in the folder 'R' and example data in the folder 'data/raw'.

You will receive further data and scripts as zipped files. Do unzip them otherwise the HTML files are not properly presented.

<a href="#top">Back to top</a>

# Learning outcomes of this introduction

The students are able to...

* Understand (2) why R, RStudio with scripts and projects, relative paths, and the packages of the `tidyverse` are a useful environment for doing analysis
* Install (3) R with RStudio
* Prepare (3) a project which is used throughout the course
* Create (3) the first two scripts within that project
* Install (3) the `tidyverse` package for data wrangling and the `here` package for relative paths
* State (1) important terms like matrix, data frame, distance matrix, community matrix, distance matrix, trait table
* Memorize (1) the example data set
...
When you start RStudio you see the following text in the console:
...
> Note for your material-and-methods section of the presentation the R version you use (not the RStudio version) and the whole citation for the reference slide of your presentation.
...
> Open your R project 'course_ecodata_my_project/my_project.Rproj'. Afterwards open within RStudio 'R/_data_preparation.R'. Within the script, install `tidyverse` on your computer. And load the package with `library(tidyverse)`. Do not forget to save, if your script name is written in blue (top left tab)
...
> Open your project 'my_project.Rproj' and install and load the libraries `here` and `readxl`.
...
# Introduction to the example data set

We will use the following data set for the illustration of analyses.
A study in the Ammer valley before and after the weir in the [floodplain Schnalz](https://www.openstreetmap.org/#map=16/47.7737/10.9615). Normally there are alluvial grey alder forests (Alnetum incanae, alliance [Alnion incanae](https://floraveg.eu/vegetation/overview/Alnion%20incanae), EUNIS habitat [T12](https://floraveg.eu/habitat/overview/T12))
...
> Open within your project folder 'course_ecodata_my_project' and have a look in the folder 'data/raw/example_course_alluvial_forest_mb'. There you find the data of the example data set.

You can load the data in RStudio:
...
> Within RStudio and your project 'my_project.Rproj', load the species data from the example data set with the script 'R/script_for_exercises'.

# GitHub and GitHub Desktop

(Not required, but recommended for those who are interested in versioning)

Go through this [tutorial](https://medium.com/@vanessa.trujillo/an-introduction-to-github-and-github-desktop-74846a76e124) of Vanessa Trujillo to be prepared for track-and-change mode in R.
