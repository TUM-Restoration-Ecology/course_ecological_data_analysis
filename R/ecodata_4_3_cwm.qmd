...
* Describe (2) how community weighted means (CWM) are calculated
* Show (3) the CWM distribution of two functional plant traits and two Ellenberg indicator values of a dataset 
...
sites_spain <- read_delim(
  here::here(
    "data", "raw", "bello_etal-2021", "chapter5", "environXplotsNE.txt"
    ),
  col_names = TRUE, delim = "\t", col_types = cols(.default = "?")
  )
...
totalabb <- species_invented %>%
  summarize(across(where(is.numeric), ~ sum(.x, na.rm = TRUE))) # across selects several columns. In this case all numeric columns
totalabb
```
...
```{r}
data <- data.frame(resCWM$FDis) %>%
  rownames_to_column(var = "id.plot") %>%
  tibble() %>%
  bind_cols(
    resCWM$CWM$SLA_log, resCWM$CWM$GrowhtForm_shrub, resCWM$CWM$LEG_1,
    resCWM$CWM$LF_Th
    ) %>%
  rename(
    "cwm.abu.sla.log" = 3,
  ...
  mutate(
    cwm.abu.sla = exp(cwm.abu.sla.log), # backtransform traits
...
write_csv(
  sites_spain, here::here(
    "data", "processed", "data_processed_sites_spain_4.3.csv"
    )
  )
```
...
This section is optional, but you should read it and do the task at the end.
...
data_traits <- traits_ammer %>%
  ...
  arrange(accepted_name) %>% # sort dataset by the column 'accepted_name'
  select(accepted_name, sla_log, growth_form_2)
```

Remove shrubs by hand which had a NA in the 'growth_from_2' column:

```{r}
data_traits2 <- data_traits %>%
  filter(
    !(accepted_name %in% c("Alnus incana", "Rosa canina", "Viburnum lantana"))
    ) %>%
  select(-growth_form_2)

data_species <- species_ammer %>%
  semi_join(data_traits2, by = "accepted_name") %>% # use only species which are no shrubs, by filter only species which are present in the data_traits table
  ...
  column_to_rownames(var = "plot_id") # this is necessary for the FD package

data_traits <- data_traits2 %>%
  column_to_rownames("accepted_name") # this is necessary for the FD package
...
data_abundance <- FD::dbFD(
  ...
  calc.CWM = TRUE, w.abun = TRUE, # abundance data = TRUE
  calc.FRic = FALSE, calc.FDiv = FALSE, corr = "none"
)
...
```{r}
write_csv(
  sites_ammer, here::here("data", "processed", "data_processed_sites_ammer_4.3.csv")
  )
```

> **Task**

* Calculate CWMs of two functional plant traits and two Ellenberg indicator values for your own dataset and present a frequency or density figure like in the examples in your appendix.
  * Remove all trees and shrub species which are in the herb layer
  * Check that species names of the traits table are rownames
  * Check that only one trait column remains in the traits table
  * Check that no NAs are in your trait column
  * If you have NAs, you have to reduce your species table as well
  * Check that the plot ID of the species table are rownmames and species names are column names
