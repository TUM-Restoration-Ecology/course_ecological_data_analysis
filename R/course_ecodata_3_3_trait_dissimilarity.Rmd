---
title: "Analysis of Ecological Data <br> 3.3 Show trait dissimilarity"
...
This script uses large parts from [Bello et al. (2021)](https://doi.org/10.1017/9781108628426)
...
traits_invented <- as.data.frame(cbind(bodysize, carnivory, colors.fuzzy))
traits_invented
```
...
```{r}
dist(traits_invented$carnivory)
```
...
```{r}
traits_invented$bodysize / max(traits_invented$bodysize, na.rm=T) #notice we have an NA to take into account
```
...
```{r}
(traits_invented$bodysize - mean(traits_invented$bodysize, na.rm=T)) / sd(traits_invented$bodysize, na.rm = T) #by hand
```
...
```{r}
dist(traits_invented$bodysize) / max(dist(traits_invented$bodysize), na.rm = T) #Gower by hand
```
...
```{r}
FD::gowdis(traits_invented[, c("bodysize", "carnivory")]) #combining 2 traits
```

By hand it would be:

```{r}
(dist(traits_invented$carnivory) + dist(traits_invented$bodysize) /  max(dist(traits_invented$bodysize), na.rm=T)) / 2
```
...
* Calculate multiple and single trait differences for your selected traits and make 
...
```{r}
colors.fuzzy2 <- prep.fuzzy(traits_invented[, 3:5], col.blocks = 3)
...
```{r}
list.traits <- ktab.list.df(list(traits_invented[, 1:2], colors.fuzzy2))
