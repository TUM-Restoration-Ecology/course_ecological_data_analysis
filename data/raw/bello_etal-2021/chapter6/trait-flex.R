trait.transform <- 
  function(spIDs, traitVals, sampleID, ...){
    matLocal <- matrix(NA, nrow=length(unique(sampleID)), ncol = length(unique(spIDs)),
                  dimnames = list(unique(sampleID), unique(spIDs)))
    for(i in 1:length(unique(sampleID))){
      spIDsAux <- subset(spIDs, sampleID==unique(sampleID)[i]) 
      spIDsAux <- droplevels(spIDsAux)
      traitValsAux <- subset(traitVals, sampleID==unique(sampleID)[i])
      localTraitMean <- tapply(traitValsAux, spIDsAux, mean, na.rm=T)
      matLocal[i, names(localTraitMean)] <- localTraitMean
    }
    matFixed <- matrix(rep(colMeans(matLocal, na.rm=T), length(unique(sampleID))), byrow=T,
                       ncol=length(unique(spIDs)), dimnames=dimnames(matLocal))
    results<-list()
    results$matLocal <- matLocal
    results$matFixed <- matFixed
    return(results)
  }

trait.CWM <- 
  function(matLocal, matFixed, commMatrix, ...){
    if(!all.equal(dim(matLocal),dim(commMatrix))|
       !all.equal(dim(matLocal),dim(matFixed))){
      stop("All the matrices provided must have the same dimensions")
    }
    if(!all(colnames(matLocal)%in%colnames(commMatrix))|
       !all(colnames(matLocal)%in%colnames(matFixed))){
      stop("All the matrices provided must have the same colnames (species names)")
    }
    if(!all(rownames(matLocal)%in%rownames(commMatrix))|
       !all(rownames(matLocal)%in%rownames(matFixed))){
      stop("All the matrices provided must have the same rownames (communities names)")
    }
    ###Reorder matrices in case it is needed:
    commMatrix <- commMatrix[rownames(matLocal), colnames(matLocal)]
    matFixed <- matFixed[rownames(matLocal), colnames(matLocal)]
    ### Turn into relative abundances
    commMatrix <- commMatrix / rowSums(commMatrix)
    
    CWMFixed <-  rowSums(commMatrix * matFixed, na.rm=T)
    CWMSpecific <-  rowSums(commMatrix * matLocal, na.rm=T)
    results<-list(specific.avg = CWMSpecific, fixed.avg = CWMFixed)
    return(results)
  }

trait.flex.anova <-
function(formula, specif.avg, fixed.avg, ...) 
{
  ...
  cons.av <- deparse(substitute(fixed.avg))
